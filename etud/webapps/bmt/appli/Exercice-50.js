...
function clickTag() {
	...
	if ($oldTag.length == 1 && $oldTag.attr('num') == $tag.attr('num')) {
		// already selected
		return
	}
...
function modifyTag() {
	...
	var name = $tag.find('input[name="name_mod"]').val()
...
function clickBookmark() {
	...
	if ($oldBm.length == 1 && $oldBm.attr('num') == $bm.attr('num')) {
		// already selected
		return
	}
...
$(function() {
	...
	// TODO 6
	// Add Listener on "add tag" when user Enter on new Tag input
	// PROF_START
